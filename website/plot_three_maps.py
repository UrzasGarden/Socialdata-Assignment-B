...
possible = [
    BASE_DIR / "INDKP107_komplet.csv",
    PROJECT_DIR / "INDKP107_komplet.csv",
    Path.cwd() / "INDKP107_komplet.csv",
    Path.cwd() / "INDKP107_komplet.csv" / "INDKP107_komplet.csv",
    Path.home() / "Downloads" / "INDKP107_komplet.csv",
    Path.home() / "Downloads" / "INDKP107_komplet.csv" / "INDKP107_komplet.csv",
]
...
lookup_candidates = [
    BASE_DIR / "GeoData" / "Places.csv",
    PROJECT_DIR / "GeoData" / "Places.csv",
    Path.cwd() / "GeoData" / "Places.csv",
]
lookup_csv_path = next((p for p in lookup_candidates if p.exists()), None)
if lookup_csv_path is None:
    raise FileNotFoundError("Could not find GeoData/Places.csv")
...
# Load geojsons
geo_roots = [BASE_DIR / "GeoData", PROJECT_DIR / "GeoData", Path.cwd() / "GeoData"]
geo_root = next((p for p in geo_roots if p.exists()), None)
if geo_root is None:
    raise FileNotFoundError("Could not find a GeoData directory")

mun_geojson_path = geo_root / "municipalities.geojson"
lands_geojson_path = geo_root / "landsdel.geojson"
region_geojson_path = geo_root / "regions.json"
...
if not region_agg.empty:
    ...
    regions_out = BASE_DIR / 'regions_map.html'
    fig_regions.write_html(regions_out)
...
if not subregion_agg.empty:
    ...
    subregions_out = BASE_DIR / 'subregions_map.html'
    fig_sub.write_html(subregions_out)
    print(f'Saved {subregions_out}')
...
if not agg.empty:
    ...
    municipalities_out = BASE_DIR / 'municipalities_map.html'
    fig_munis.write_html(municipalities_out)
    print(f'Saved {municipalities_out}')
